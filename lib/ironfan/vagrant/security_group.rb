module Ironfan
  module Vagrant
    class SecurityGroup < DslObject
      ...
      def self.get_all
        if (@@vpc)
          groups_list = Ironfan::Vagrant::Cloud.fog_connection.security_groups.all({ "group-name" => "#{@@vpc}-*" })
        else
          groups_list = Ironfan::Vagrant::Cloud.fog_connection.security_groups.all
        end
      ...
      def self.get_or_create(group_name, description)
        ...
        fog_group = all[group_name] || Ironfan::Vagrant::Cloud.fog_connection.security_groups.get(group_name)
        unless fog_group
          self.step(group_name, "creating (#{description})", :green)
          fog_group = all[group_name] = Ironfan::Vagrant::Cloud.fog_connection.security_groups.new(:name => group_name, :description => description, :connection => Ironfan::Vagrant::Cloud.fog_connection, :vpc_id => @@vpc)
          fog_group.save
        end
        fog_group
      end
